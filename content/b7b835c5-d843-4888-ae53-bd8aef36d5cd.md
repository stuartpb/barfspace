...
The SACRED Properties, which have Issues and History, should be **merged into a page under the Spec Lab, issues copied in, tombstoned with a link to the notes page, given a CNAME, and archived**.
...
This should be **copied** into a subnote of a new "Game Ideas" note under the Projects nexus, then **tombstoned and archived**.
...
Since this is a repo I want to keep OpenSCAD files and/or other assets (ie. SVG) in, I'm going to **keep** this repo, but **making a note for it** under the "Game Ideas" project collecting my brainstorming thoughts for it, which I might then **link in to the repo's README.md**.

## crane

Since this is a bunch of brainstorming for a project that I think is already out there, I'm gonna free up this name by **merging the content in** as a project page and then **deleting the repo** (checking if it's on Cloud9 first, which it shouldn't be).

I was thinking of copying, since the history is mostly just merges, but I figured it's worth preserving the file's history if I'm goin to blow away the repo.

## writing-the-ship

I'm going to **copy the README and issues into a new Game Projects note**, then **tombstone and archive**.
